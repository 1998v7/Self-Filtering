    ...
    def __call__(self, outputs, targets, args, mode='warm_up'):
        ...
        if mode == 'warm_up':
            ...
            for i in range(values.size(0)):
                x = values[i]
                latent_lam[i] = max(1 - min(x) / max(x) - args.T, 0.0)
            conf_penalty = (F.cross_entropy(outputs, latent_label, reduction='none') * latent_lam).mean()
            ...
            for i in range(bs):
                ...
                confident = 1 - confident / max_ - args.T
                mask = (confident >= 0.0)
...
def warmup(epoch, net, optimizer, dataloader, args):
    ...
    for batch_idx, (inputs, labels, path) in enumerate(dataloader):
        ...
        optimizer.zero_grad()
        outputs= net(inputs)
        if args.noise_mode == 'instance' or args.noise_mode == 'pair':  # penalize confident prediction for asymmetric noise
...
def eval_train(model, memory_bank, eval_loader, args, epoch, test_log):
            ...
            for b in range(inputs.size(0)):
                ...
                if selection:
                    ...
                    if memory_bank_last[sample_index] > result[sample_index] and result[sample_index] == 0:
                        # the prediction of this sample changes from correct to wrong, thus it is fluctuation
                        # we consider it as the clean with 0%
                        fluctuation_[sample_index] = 0
                    else:
                        # we consider it as the clean with result[sample_index]% (it is the model output prob)
                        fluctuation_[sample_index] = result[sample_index]
    ...
    if selection:
        ...
        return np.array(fluctuation_), memory_bank
