...
class cifar_dataloader():
    ...
    def run(self, mode, pred=[], prob=[], test_log=None, epoch=None, id_list=None):
        if mode == 'warmup':
            all_dataset = cifar_dataset(self.train_data, self.train_label, self.noise_label, self.transform_train, mode='all', strong_transform=None,
                                            pred=pred, probability=prob, test_log=test_log, id_list=id_list)
            ...
            labeled_dataset = cifar_dataset(self.train_data, self.train_label, self.noise_label, self.transform_train, mode='labeled',
                                            strong_transform=None, pred=pred, probability=prob, test_log=test_log, id_list=id_list)
            ...
            test_dataset = cifar_dataset(self.test_data, self.train_label, self.test_label, self.transform_train, mode='test', strong_transform=None, pred=pred, probability=prob)
            test_loader = DataLoader(dataset=test_dataset, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)
            return test_loader

        elif mode == 'eval_train':
            eval_dataset = cifar_dataset(self.train_data, self.train_label, self.noise_label, self.transform_train, mode='all', strong_transform=None, pred=pred, probability=prob)
